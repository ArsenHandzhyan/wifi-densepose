#!/usr/bin/env python3
"""
V45 Three-Class Model: empty / static / motion

Builds on V44 binary (F1=1.0) by splitting 'present' into static vs motion.
Uses ONLY Tenda epoch recordings (no snapshots) for clean feature distribution.

Data sources:
  - empty:  empty garage recordings (V44 empty patterns + v43 20min baselines)
  - static: marker1-8, center, door standing, occupied static positions
  - motion: walking captures (center, passage, door, transitions)
"""
...
OUTPUT_DIR = PROJECT / "output" / "v45_3class_motion"
...
STATIC_PATTERNS = [
    # 1-person markers (standing still at known positions)
...
MOTION_PATTERNS = [
    # Walking captures (center, passage, door, transitions)
    "walking_v44_20260401_010017_walk_*_chunk*.ndjson.gz",
    # 2-person freeform (movement)
...
def extract_features(data: dict) -> np.ndarray | None:
    """8 features per node × 7 nodes = 56 features."""
    features = []
    active = 0
    for ip in NODE_IPS:
...
def main():
    print("=" * 60)
    print("V45 Three-Class: empty / static / motion")
    print("=" * 60)
    ...
    if len(counts) < 3:
        ...
        if "static" not in counts:
            print("   MISSING: static class")
        if "motion" not in counts:
            print("   MISSING: motion class")
    ...
    # Save to V45 own dir
    v45_path = OUTPUT_DIR / "v45_3class_motion.pkl"
    with open(v45_path, "wb") as f:
    ...
    for path in [OUTPUT_DIR / "v45_meta.json", PROJECT / "output" / "epoch4_v41_model" / "v41_position_classifier_meta.json"]:
    ...
    print(f"   Meta: {OUTPUT_DIR / 'v45_meta.json'}")
    print("\nDone!")
    return 0
